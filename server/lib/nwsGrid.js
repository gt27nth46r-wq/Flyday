...
// Some NWS forecast offices occasionally publish a sentinel/error value
// (e.g. -9999) in a gridpoint element instead of simply omitting a missing
// reading. Unlike a genuinely absent entry (expandToHourly's map just has no
// key for that hour, handled fine elsewhere), a garbage numeric value flows
// straight through unless something checks it - which is what caused a
// reported density-altitude bug: a temperature sentinel, multiplied by 120
// in the density-altitude formula, produced a result in the millions of
// feet. This rejects anything outside what's physically possible on Earth,
// treating it the same as "no data" rather than letting it corrupt
// downstream math.
function validOrNull(value, min, max) {
...
export function buildHourlyTimeline(gridProps, days = 7) {
  ...
  for (let h = 0; h < hours; h++) {
    ...
    const pa = validOrNull(pressureMap.get(key), 30000, 110000); // Pa - covers extreme-elevation stations through record sea-level highs
    const wk = validOrNull(windMap.get(key), 0, 400); // km/h - generous ceiling well above any recorded surface wind
    const gk = validOrNull(gustMap.get(key), 0, 500);
    const cm = ceilingMap.get(key);
    const vm = visMap.get(key);
    const ice = validOrNull(iceMap.get(key), 0, 500); // mm
    ...
    timeline.push({
      ...
      // Whether the gridpoint published ANY ceiling entry for this hour -
      // distinct from the value itself. A missing entry means the forecast
      // office simply didn't include this hour, not that the sky is clear;
      // only a real entry (however high) counts as "reported."
      ceilingKnown: ceilingMap.has(key),
      visibilitySm: vm != null ? vm / 1609.34 : null,
      tempC: validOrNull(tempMap.get(key), -75, 58), // Earth's actual extremes are about -89.2C to 56.7C
      dewpointC: validOrNull(dewpointMap.get(key), -75, 45), // dewpoint can't meaningfully exceed temperature's own range
