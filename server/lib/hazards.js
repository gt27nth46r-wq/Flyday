...
//
// Icing and turbulence AIRMETs are routinely issued over huge areas with a
// base well aloft (5,000-18,000ft+) - completely irrelevant to a calm, clear
// day at pattern altitude. Without an altitude check, a broad icing AIRMET
// based at FL180 would incorrectly ground a perfect local VFR day. Convective
// and IFR/obscuration hazards don't get this filter - a thunderstorm affects
// the whole column, and IFR/obscuration means a surface ceiling/visibility
// problem by definition.
const ALTITUDE_RELEVANCE_CEILING_FT = 6000; // MSL - generous cutoff for "matters to a local VFR flight"
...
export function activeHazardsAtPoint(airSigmets, lat, lon, atMs = Date.now()) {
  ...
  for (const item of airSigmets) {
    ...
    const altitudeLow = Number(item.altitudeLow1);
    const hasLowBase = !Number.isFinite(altitudeLow) || altitudeLow <= ALTITUDE_RELEVANCE_CEILING_FT;

    if (hazard.includes("ICE")) {
      if (hasLowBase) active.icing = true;
    } else if (hazard.includes("TURB")) {
      if (hasLowBase) active.turbulence = true;
    } else if (hazard.includes("CONVECTIVE") || hazard.includes("TS")) {
      active.convective = true;
    } else if (hazard.includes("IFR") || hazard.includes("MTN OBSC") || hazard.includes("OBSC")) {
      active.ifrOrObscuration = true;
    }
  }
