...
export function evaluateChecklist(input, opts) {
  ...
  // Daytime only: this formula estimates where a sun-driven thermal
  // condenses, which isn't a meaningful concept overnight - and overnight
  // radiative cooling naturally narrows the temp/dew point spread anyway,
  // which would otherwise make this read as a false low-cloud-base warning
  // on an ordinary calm night.
  const cloudBaseApplicable = input.isDaytime !== false;
  items.push({
    key: "cloudBase",
    label: "Est. convective cloud base",
    value: cloudBaseApplicable && input.cloudBaseFt != null ? `${input.cloudBaseFt.toLocaleString()} ft` : null,
    tier: cloudBaseApplicable ? tierFromBands(input.cloudBaseFt, (v) => v > cloudBaseGreenFt, (v) => v >= cloudBaseYellowFt) : "NA",
    critical: false,
  });
