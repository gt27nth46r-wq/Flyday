...
export function evaluateChecklist(input, opts) {
  const {
    ...
    fogWindMinKt = 2, // light wind actually raises fog risk (mixes moisture through a deeper layer)
    fogWindMaxKt = 10, // above this, mixing keeps the surface from cooling to saturation
    fogSkyClearMaxPct = 30, // radiational cooling needs a clear-ish sky to work
  ...
  // 5. Ceiling - critical, same reasoning as visibility. Comes from the real
  // METAR for the current hour, and from the NWS gridpoint's own forecast
  // ceilingHeight element (a real forecaster-reviewed product, not a raw
  // guess) for hours beyond that - which itself already represents "ceiling"
  // in the proper aviation sense (base of the lowest broken/overcast layer,
  // or vertical visibility into an obscuration), not "any cloud base"
  // including scattered/few layers. Either way, a null ceilingFt only reads
  // as Good ("no ceiling reported" - i.e. clear) when `ceilingKnown` is
  // true; when an hour simply has no entry at all, it reads N/A rather than
  // guessing "clear" from the absence of data.
  items.push({
    key: "ceiling",
    label: "Ceiling",
    value: !input.ceilingKnown ? null : input.ceilingFt != null ? `${Math.round(input.ceilingFt)} ft` : "unlimited",
    tier: !input.ceilingKnown ? "NA" : input.ceilingFt == null ? "GREEN" : tierFromBands(input.ceilingFt, (v) => v >= ceilingGreenFt, (v) => v >= ceilingYellowFt),
    critical: true,
  });
  ...
  // Overnight/pre-dawn hours also factor in radiation-fog mechanics: a
  // light breeze (2-10kt) actually *increases* fog risk by mixing moisture
  // through a deeper layer - calm air lets moisture stay in a very shallow
  // layer, and above ~10kt, mixing keeps the surface from cooling to
  // saturation - and radiational cooling needs a clear-ish sky to work at
  // all. When the spread is already in the 3-5°C caution band and both of
  // those line up, that's escalated to Bad. Below 3°C is already Bad from
  // the spread alone; above 5°C is dry enough that wind/sky don't move it.
  // This mechanism doesn't apply in daytime (solar heating prevents fog
  // regardless of spread), so it's skipped then.
  let dewpointTier = tierFromBands(input.dewpointSpreadC, (v) => v > dewpointSpreadGreenC, (v) => v >= dewpointSpreadYellowC);
  let fogNote = "";

  if (input.isDaytime === false && dewpointTier === "YELLOW") {
    const windFavorsFog = input.windSpeedKt != null && input.windSpeedKt >= fogWindMinKt && input.windSpeedKt <= fogWindMaxKt;
    const skyFavorsFog = input.skyCoverPct != null && input.skyCoverPct <= fogSkyClearMaxPct;
    if (windFavorsFog && skyFavorsFog) {
      dewpointTier = "RED";
      fogNote = " - fog risk (light wind + clear sky favor radiation fog)";
    }
  }
  ...
  items.push({
    ...
    value: input.dewpointSpreadC != null ? `${input.dewpointSpreadC.toFixed(1)} °C${fogNote}` : null,
    tier: dewpointTier,
    critical: false,
  });
  ...
  // offset. Displayed in inches of mercury (the unit pilots actually read
  // off an altimeter/METAR), with an arrow for which way the 3-hour trend
  // is pointing.
  ...
  const inHg = input.pressureHpa != null ? (input.pressureHpa * 0.0295299830714).toFixed(2) : null;
  const trendArrow =
    input.pressureTrendHpa3h == null ? "" : input.pressureTrendHpa3h >= 0.5 ? " ↑" : input.pressureTrendHpa3h <= -0.5 ? " ↓" : " →";

  items.push({
    key: "pressurePattern",
    label: "Pressure pattern",
    value: inHg != null ? `${inHg}"${trendArrow}${tightIsobars ? " (tight gradient)" : ""}` : null,
    tier: pressureTier,
    critical: true,
  });
