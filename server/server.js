...
function hourChecklistInput(hour, tafSignals) {
  return {
    ...
    ceilingKnown: !!hour.ceilingReported,
...
app.get("/api/forecast", async (req, res) => {
  try {
    ...
    if (!bundle) {
      ...
      // Ceiling is shown only for the current hour, sourced from the real
      // METAR (see the override below) - not the NWS model's forecast
      // ceilingHeight element, which is a guess about the future rather
      // than a report of actual clouds. Every hour, including "now," starts
      // cleared here; only a real METAR (below) can populate it, so if
      // there's no METAR to report one, it stays N/A rather than quietly
      // falling back to a model guess. `ceilingReported` tracks whether an
      // hour actually got a real report, since a null ceilingFt is
      // ambiguous on its own (could mean "no ceiling reported" - i.e.
      // clear skies - or "we simply don't track this hour").
      for (const hour of timeline) {
        hour.ceilingFt = null;
        hour.ceilingReported = false;
      }
      ...
      // where the METAR reports them. Forecast-only fields (precip chance,
      // ice accumulation, lightning activity level, pressure trend/gradient)
      // are left as the model's values, since METAR doesn't report those.
      ...
      for (const day of dailyGrid) {
        ...
        day.ceilingKnown = hoursForDaySummary.some((h) => h.ceilingReported);
