...
function dayChecklistInput(day, tafSignals) {
  return {
    ...
    cloudBaseFt: min(day.hourly.map((h) => h.cloudBaseFt)), // worst case = lowest estimated base
    ...
    tafVolatile: tafSignals.volatile,
    tafHazard: tafSignals.hazard,
    hazardsChecked: day.hourly.some((h) => h.hazardsChecked),
    turbulenceHazard: day.hourly.some((h) => h.turbulenceHazard),
    icingHazard: day.hourly.some((h) => h.icingHazard),
    convectiveHazard: day.hourly.some((h) => h.convectiveHazard),
  };
}
  ...
  try {
    ...
    if (!bundle) {
      ...
      for (const day of dailyGrid) {
        // Day/night, computed locally (no API needed) from the airport's coordinates.
