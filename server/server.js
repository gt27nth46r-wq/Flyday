...
import { lookupStation, resolveGridPoint, HttpError } from "./lib/geo.js";
import { fetchGridForecast, buildHourlyTimeline, summarizeDailyFromTimeline } from "./lib/nwsGrid.js";
import { fetchLatestMetar, fetchTaf, worstCategoryForDay } from "./lib/aviationWeather.js";
import { rateDay } from "./lib/rating.js";
...
app.get("/api/forecast", async (req, res) => {
  try {
    ...
    if (!bundle) {
      ...
      const [gridProps, metar, taf] = await Promise.all([
    ...
    const days = bundle.dailyGrid.map((day) => {
      const rating = rateDay(day, { windThresholdKt, pressureThresholdHpa, ceilingThresholdFt, visibilityThresholdSm });
      ...
      return { ...day, ...rating, tafFlightCategory: tafCategory, densityAltitudeFt: densityAltFt };
    });
    ...
    res.json({
      ...
      thresholds: { windThresholdKt, pressureThresholdHpa, ceilingThresholdFt, visibilityThresholdSm },
      metar: bundle.metar,
      ...
      sources: [
        ...
        "https://www.weather.gov/documentation/services-web-api (7-day gridpoint forecast: pressure, wind, ceiling, visibility, temperature)",
      ],
    });
  } catch (err) {
