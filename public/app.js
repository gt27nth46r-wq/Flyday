...
function bestFlyingWindow(hourFinals) {
  ...
  const bestLabel = daytime.some((h) => h.label === "GOOD")
    ? "GOOD"
  ...
  for (const h of daytime) {
    if (h.label === bestLabel) {
      current = current ? { ...current, end: h, length: current.length + 1 } : { start: h, end: h, length: 1 };
  ...
  if (!best) return null;
  return { startIso: best.start.hour.timeIso, endIso: best.end.hour.timeIso, label: bestLabel };
}
