...
// label, same philosophy as the rest of the app.
function majorityLabel(hourFinalsSubset) {
  ...
  if (counts.BAD === maxCount) return "BAD";
  if (counts.MARGINAL === maxCount) return "MARGINAL";
  return "GOOD";
}
...
function hourDetailHtml({ hour, label, checklist }, timeZone) {
  ...
  return `
    <div class="hour-detail-head">
      <div class="hour-detail-time">${timeFmt.format(new Date(hour.timeIso))} ${sky}</div>
      <div class="rating">${label}</div>
    </div>
    ${readoutHtml}
    ${checklistTableHtml(checklist)}
  `;
}
...
function renderDays(data) {
  ...
  for (const day of data.days) {
    ...
    const dayGrade = majorityLabel(dayHours);
    const nightGrade = majorityLabel(nightHours);
    const cardOverall = worseLabel(dayGrade, nightGrade);
    ...
    card.innerHTML = `
      ...
      <div class="day-night-grades">
        <div class="grade-pill ${dayGrade}"><span class="grade-icon">☀</span><span class="grade-label">Day</span><span class="grade-value">${dayGrade}</span></div>
        <div class="grade-pill ${nightGrade}"><span class="grade-icon">🌙</span><span class="grade-label">Night</span><span class="grade-value">${nightGrade}</span></div>
      </div>
    ...
    function selectHour(idx) {
      ...
      detailPanel.innerHTML = hourDetailHtml(hourFinal, data.timeZone);
