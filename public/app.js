...
// The best contiguous stretch of daytime hours to fly - the longest run of
// hours sharing whichever rating is best available that day (all-Good if
// there is one, otherwise the longest Marginal run, etc.), not just the
// single best hour. Ties in length go to whichever run happens first, since
// there's no reason to prefer one over the other otherwise.
// How many contributing problems an hour's checklist has - Red items count
// double, Yellow items count once, Green/N-A don't count. Used to pick the
// best stretch among hours that all carry the same Good/Marginal/Bad label,
// since "Marginal" can still mean meaningfully better or worse hour to hour.
function hourBadnessScore(checklist) {
  let score = 0;
  for (const item of checklist) {
    if (item.tier === "RED") score += 2;
    else if (item.tier === "YELLOW") score += 1;
  }
  return score;
}

// The best contiguous stretch of daytime hours to fly. Never a Bad window -
// if every daytime hour is Bad, there's no "best time," so this returns
// null rather than picking the least-bad Bad stretch. If there's a Good
// stretch anywhere, the longest one wins. If the whole day is Marginal (no
// Good hours at all), it's not just about picking the longest Marginal
// run - it's whichever contiguous run has the best (lowest) average
// badness score, since a short, calmer stretch is a better answer than a
// long stretch of worse Marginal hours.
function bestFlyingWindow(hourFinals) {
  const daytime = hourFinals.filter((h) => h.hour.isDaytime === true && h.label !== "UNKNOWN");
  if (!daytime.length) return null;

  const targetLabel = daytime.some((h) => h.label === "GOOD")
    ? "GOOD"
    : daytime.some((h) => h.label === "MARGINAL")
      ? "MARGINAL"
      : null; // every hour is Bad - never suggest a Bad window as "best"
  ...
  for (const h of daytime) {
    if (h.label === targetLabel) {
      const score = hourBadnessScore(h.checklist);
      if (current) {
        current.end = h;
        current.length++;
        current.totalScore += score;
      } else {
        current = { start: h, end: h, length: 1, totalScore: score };
      }
    } else {
  ...
  const best =
    targetLabel === "GOOD"
      ...
      : runs.reduce((a, b) => (b.totalScore / b.length < a.totalScore / a.length ? b : a));

  return { startIso: best.start.hour.timeIso, endIso: best.end.hour.timeIso, label: targetLabel };
}
...
// Pressure is shown in inches of mercury (what pilots actually read off an
// altimeter/METAR), with an arrow for which way the 3-hour trend is
// pointing - same ±0.5 hPa/3h threshold as the backend's checklist item
// (server/lib/checklist.js), kept in sync by hand.
function pressureReadout(hpa, trendHpa3h) {
  if (hpa == null) return "—";
  const inHg = (hpa * 0.0295299830714).toFixed(2);
  const arrow = trendHpa3h == null ? "" : trendHpa3h >= 0.5 ? " ↑" : trendHpa3h <= -0.5 ? " ↓" : " →";
  return `${inHg}"${arrow}`;
}
...
function renderCurrent(data) {
  ...
  if (data.tafSubstitute) {
    notices.push(`${data.station.icao} doesn't have its own TAF - the flight-category and TAF-trend rows are using ${data.tafSubstitute.icao}'s TAF instead, ${Math.round(data.tafSubstitute.distanceNm)} nm away.`);
  }
...
function hourStripHtml(hourFinals, timeZone) {
  ...
  const hasAnyTaf = hourFinals.some(({ hour }) => hour.tafFlightCategory);
  ...
  const blocks = hourFinals
    .map(({ hour, label }, idx) => {
      ...
      const cat = hour.tafFlightCategory;
      const title = `${t}${hour.isDaytime === false ? " (night)" : hour.isDaytime ? " (day)" : ""}: ${label}${cat ? ` · TAF ${cat}` : ""}${hour.windSpeedKt != null ? ` · ${fmt(hour.windDirDeg)}°/${fmt(hour.windSpeedKt)}kt` : ""} - tap for details`;
      ...
      return `<div class="hour-col">
        ...
        ${hasAnyTaf ? `<span class="flightcat-block ${cat || "NONE"}" title="${title}" data-idx="${idx}" role="button" tabindex="0"></span>` : ""}
        ${showLabel ? `<span class="hour-label">${compactLabel}</span>` : ""}
      </div>`;
    })
    .join("");
  const tafLegend = hasAnyTaf
    ? `<div class="flightcat-legend">Flight category (TAF): <span class="flightcat-dot VFR"></span>VFR <span class="flightcat-dot MVFR"></span>MVFR <span class="flightcat-dot IFR"></span>IFR <span class="flightcat-dot LIFR"></span>LIFR</div>`
    : "";
  return `<div class="hour-strip-wrap"><div class="hour-strip" aria-label="Hour by hour outlook, all 24 hours, shaded for day and night - tap an hour for details">${blocks}</div></div>${tafLegend}`;
}
...
function hourlyTableHtml(hourFinals, timeZone) {
  ...
  const rows = hourFinals
    ...
    .map(({ hour, label, idx }) => {
      ...
      return `<tr data-idx="${idx}">
        ...
        <td>${hour.tafFlightCategory ? `<span class="flightcat-dot ${hour.tafFlightCategory}"></span>${hour.tafFlightCategory}` : "—"}</td>
  ...
  return `<details class="hourly">
    ...
    <div class="table-scroll"><table class="hourly-table hourly-table-clickable">
      <thead><tr><th>Time</th><th>Flight cat</th><th>Wind</th><th>Ceil ft</th><th>Vis sm</th><th>Trend/3h</th><th>Gradient/100km</th></tr></thead>
      <tbody>${rows}</tbody>
    </table></div>
  </details>`;
}
...
function checklistTableHtml(checklist, tier) {
  ...
  // "Full checklist" now always means the full checklist - every item shows
  // when expanded. When the rating is Marginal/Bad, the contributing items
  // are just sorted to the top for a quick scan, not hidden from the rest.
  const tierOrder = { RED: 0, YELLOW: 1, GREEN: 2, NA: 3 };
  const rowsData = focusOnProblems ? [...checklist].sort((a, b) => tierOrder[a.tier] - tierOrder[b.tier]) : checklist;
  ...
  const note = focusOnProblems && problems.length
    ? `<p class="checklist-focus-note">${problems.length} item${problems.length === 1 ? "" : "s"} contributing to this rating, listed first below.</p>`
    : "";
...
function hourDetailHtml({ hour, label, tier, checklist }, timeZone, persistenceText) {
  ...
  const readouts = [
    ["Flight category (TAF)", hour.tafFlightCategory ? `<span class="flightcat-dot ${hour.tafFlightCategory}"></span>${hour.tafFlightCategory}` : null],
    ["Wind", hour.windSpeedKt != null ? `${fmt(hour.windDirDeg)}° @ ${fmt(hour.windSpeedKt)} kt${hour.windGustKt ? ` G${fmt(hour.windGustKt)}` : ""}` : null],
    ["Ceiling", !hour.ceilingKnown ? null : hour.ceilingFt != null ? `${fmt(hour.ceilingFt)} ft` : "unlimited"],
    ...
    ["Pressure", hour.pressureHpa != null ? pressureReadout(hour.pressureHpa, hour.pressureTrendHpa3h) : null],
...
function renderDays(data) {
  ...
  for (const day of data.days) {
    ...
    const bestWindow = bestFlyingWindow(hourFinals);
    const bestWindowFmt = new Intl.DateTimeFormat("en-US", { hour: "numeric", timeZone: data.timeZone });
    const bestWindowHtml = bestWindow
      ? `<div class="best-window ${bestWindow.label}"><span class="best-window-icon">🕐</span><span>Best time to fly: <strong>${bestWindowFmt.format(new Date(bestWindow.startIso))}–${bestWindowFmt.format(new Date(new Date(bestWindow.endIso).getTime() + 3600000))}</strong> (${bestWindow.label})</span></div>`
      : "";
    ...
    card.innerHTML = `
      ...
      <div class="readout"><span class="k">Pressure</span><span>${pressureReadout(day.avgPressureHpa, day.minPressureTrendHpa3h)}</span></div>
...
async function loadForecast() {
  ...
  try {
    ...
    renderDays(data);
    updateProgchart();
    setStatus(`Updated ${new Date(data.fetchedAt).toLocaleTimeString()} · wind ≤ ${data.thresholds.windThresholdKt} kt · pressure ≥ ${(data.thresholds.pressureThresholdHpa * 0.0295299830714).toFixed(2)}"`);
  } catch (err) {
  ...
  try {
    ...
    for (const r of results) {
      ...
      html += `<tr><td>${r.icao}</td>${
        r.data.days.map((d) => `<td><span class="cell ${d.label}" title="${pressureReadout(d.avgPressureHpa, d.minPressureTrendHpa3h)}, ${fmt(d.maxWindKt)} kt">${d.label}</span></td>`).join("")
      }</tr>`;
    }
    html += "</tbody></table>";
