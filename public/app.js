...
// Mirrors server-side applyRunwayAlignment() in lib/rating.js - kept in sync
// by hand since runway heading never leaves the browser. Downgrades an
// hour's server-computed label to BAD if the wind is more than 15 degrees
// off the runway's line (0-90 deg, since a runway flies from either end).
function finalHourLabel(hour, runwayHeadingDeg) {
  if (runwayHeadingDeg == null || hour.windDirDeg == null || hour.label == null) return hour.label;
  if (hour.windSpeedKt != null && hour.windSpeedKt < 3) return hour.label; // direction is noisy near calm

  const raw = Math.abs((((hour.windDirDeg - runwayHeadingDeg) % 180) + 180) % 180);
  const angleOffRunway = raw > 90 ? 180 - raw : raw;

  return angleOffRunway > 15 ? "BAD" : hour.label;
}
...
// showing the best/worst times to fly that day at a glance.
function hourStripHtml(day, runwayHeadingDeg, timeZone) {
  const timeFmt = new Intl.DateTimeFormat("en-US", { hour: "numeric", timeZone });
  const daylightHours = day.hourly.filter((h) => {
    const localHour = Number(
      new Intl.DateTimeFormat("en-US", { hour: "numeric", hour12: false, timeZone }).format(new Date(h.timeIso))
    );
    return localHour >= 6 && localHour <= 20 && h.label != null;
  });

  if (!daylightHours.length) return "";

  const blocks = daylightHours
    .map((h) => {
      const label = finalHourLabel(h, runwayHeadingDeg);
      const t = timeFmt.format(new Date(h.timeIso));
      const title = `${t}: ${label}${h.windSpeedKt != null ? ` · ${fmt(h.windDirDeg)}°/${fmt(h.windSpeedKt)}kt` : ""}`;
...
function hourlyTableHtml(day, runwayHeadingDeg, timeZone) {
  const timeFmt = new Intl.DateTimeFormat("en-US", { hour: "numeric", timeZone });
  const rows = day.hourly
    ...
    .map((h) => {
      ...
      return `<tr>
        ...
        <td>${h.pressureTrendHpa3h != null ? fmt(h.pressureTrendHpa3h, 1) : "—"}</td>
        <td>${h.pressureGradientHpa100km != null ? fmt(h.pressureGradientHpa100km, 1) : "—"}</td>
      </tr>`;
    })
    .join("");
...
function renderDays(data) {
  ...
  for (const day of data.days) {
    ...
    card.innerHTML = `
      ...
      ${hourStripHtml(day, runwayHeadingDeg, data.timeZone)}
      ...
      ${day.lowMovingIn ? `<div class="readout warn"><span class="k">⚠ Low pressure</span><span>moving in</span></div>` : ""}
      ${day.tightIsobars ? `<div class="readout warn"><span class="k">⚠ Isobars</span><span>tightly packed</span></div>` : ""}
      ${day.tafFlightCategory ? `<span class="flightcat taf-badge ${day.tafFlightCategory}">TAF ${day.tafFlightCategory}</span>` : ""}
      ${hourlyTableHtml(day, runwayHeadingDeg, data.timeZone)}
    `;
